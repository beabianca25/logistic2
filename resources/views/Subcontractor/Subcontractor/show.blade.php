                        ...
                        <div class="card-body row">
                            @foreach ($subcontractor->requirements as $requirement)
                                <div class="col-md-6">
                                    ...
                                    <p><strong>Preffered Payment:</strong> {{ $requirement->preferred_payment_terms }}</p>
                                    <p><strong>Start Date Availability:</strong>
                                        {{ $requirement->start_date_availability }}</p>
                                    <p><strong>Estimated Completion Time:</strong>
                                        {{ $requirement->estimated_completion_time }}</p>
                                    <p><strong>Resources:</strong> {{ $requirement->resources_required }}</p>
                                </div>
                                <div class="col-md-6">
                                    <p><strong>Insurance Coverage:</strong> {{ $requirement->insurance_coverage }}</p>
                                    <p><strong>Certification/Licence:</strong>
                                        {{ $requirement->certifications_or_licenses }}</p>
                                </div>
                            @endforeach
                        </div>
                    ...
                    <div class="card mb-3">
                        ...
                        <div class="card-body">
                            @foreach ($subcontractor->attachments as $attachment)
                                <p><strong>Portfolio Samples:</strong> <a
                                        href='{{ asset('storage/' . $attachment->portfolio_samples) }}'
                                        target='_blank'>View File</a> </p>
                                <p><strong>Business License:</strong> <a
                                        href='{{ asset('storage/' . $attachment->business_licences) }}'
                                        target='_blank'>View File</a> </p>
                                <p><strong>Aggrement knowledge:</strong>
                                    {{ $attachment->aggrement_knowledge ? 'Yes' : 'No' }}</p>
                                <p><strong>Signature:</strong> <a href='{{ asset('storage/' . $attachment->signature) }}'
                                        target='_blank'>View File</a> </p>
                                <p><strong>Submission Date:</strong> {{ $attachment->submission_date }}</p>
                            @endforeach
                        </div>
                    </div>
                </div>
