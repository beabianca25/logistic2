    ...
    <div class="container">
        ...
        <div class="card">
            <div class="card-body">
                ...
                <p><strong>Departure Time:</strong> {{ $trip->departure_time->format('Y-m-d H:i') }}</p>
                <p><strong>Expected Arrival Time:</strong> {{ $trip->expected_arrival_time->format('Y-m-d H:i') }}</p>
                <p><strong>Status:</strong> {{ ucfirst($trip->status) }}</p>
                <a href="{{ route('trips.index') }}" class="btn btn-primary">Back to List</a>
            </div>
        </div>
    </div>
@endsection
