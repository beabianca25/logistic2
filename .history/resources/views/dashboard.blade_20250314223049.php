        ...
        <div class="row">
            ...
            <div class="col-lg-3 col-6">
                <div class="small-box bg-cyan">
                    <div class="inner">
                        <h3>{{ $bidCount }}</h3> <!-- Directly show bid count -->
                        <p>Bid Count</p>
                    </div>
                    <div class="icon">
            ...
            <script>
                function fetchBidCount() {
                    $.ajax({
                        ...
                        method: "GET",
                        success: function(response) {
                            let bidCount = parseInt(response.bidCount) || 0;
                            $("#bidCount").html(bidCount);
                        },
                        error: function(error) {
                            console.log("Error fetching bid count", error);
                        }
                    });
                }

                // Fetch bid count every 5 seconds
                setInterval(fetchBidCount, 1000);
                fetchBidCount(); // Initial fetch
            </script>
        ...
        <div class="row">
            ...
            <script>
                $(document).ready(function() {
                    ...
                    $.ajax({
                        url: "{{ route('report.bookingStatusData') }}", // Updated route
                        method: "GET",
                        success: function(response) {
                            console.log("Response Data:", response);
                            $('#loadingIndicator').hide();
            
                            if (!response || !response.dates || !response.statusCounts) {
