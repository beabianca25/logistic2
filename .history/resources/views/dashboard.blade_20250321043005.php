    ...
    <div class="container" style="font-family: sans-serif; font-size: small;">
        ...
        <div class="row">
            <style>
                ...
                .card-tools i {
                    font-size: 14px; /* Reduce icon size for better proportion */
                }
                ...
                <div class="card floating-card">
                    ...
                    <div class="card-body">
                        <div class="chart">
                            ...
                            <div id="loadingIndicator" style="text-align:center; margin-top: 10px; font-size: 14px;">Loading...</div>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    $.ajax({
                        ...
                        success: function (response) {
                            ...
                            var barChartData = {
                                labels: months,
                                datasets: [
                                    { label: 'Pending', backgroundColor: 'rgba(255, 206, 86, 0.8)', data: statusCounts['Pending'] },
                                    { label: 'Approved', backgroundColor: 'rgba(75, 192, 192, 0.8)', data: statusCounts['Approved'] },
                                    { label: 'Scheduled', backgroundColor: 'rgba(54, 162, 235, 0.8)', data: statusCounts['Scheduled'] },
                                    { label: 'Ongoing', backgroundColor: 'rgba(153, 102, 255, 0.8)', data: statusCounts['Ongoing'] },
                                    { label: 'Completed', backgroundColor: 'rgba(0, 255, 0, 0.8)', data: statusCounts['Completed'] },
                                    { label: 'Cancelled', backgroundColor: 'rgba(255, 99, 132, 0.8)', data: statusCounts['Cancelled'] }
                                ]
                            };
            ...
            <style>
                .floating-chat {
                    box-shadow: 0px 4px 10px rgba(0, 0, 0, 0.15); /* Floating effect */
                    border-radius: 12px; /* Smooth rounded corners */
                    background: #fff;
                    transition: 0.3s ease-in-out;
                }
                ...
                .direct-chat-messages {
                    font-size: 13px; /* Smaller chat text */
                    padding: 10px;
                    background: #f9f9f9;
                    border-radius: 8px;
                }
        ...
        <style>
            ...
            .users-list {
                font-size: 13px; /* Smaller font size for user list */
            }
                                ...
                                <tbody>
                                    @if (isset($bids) && $bids->count() > 0)
                                        @foreach ($bids as $bid)
                                            <tr>
                                                <td>
                                                    <a href="{{ route('bid.show', $bid->id) }}">BID{{ $bid->id }}</a>
                                                </td>
                    ...
                    <div class="card-body p-0">
                        <ul class="users-list clearfix">
                            @foreach ($users as $user)
                                <li>
                                    <img class="direct-chat-img" src="{{ asset('images/JVD-removebg-preview.png') }}" alt="User Image">
