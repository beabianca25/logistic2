...
document.addEventListener("DOMContentLoaded", function () {
    document.addEventListener("DOMContentLoaded", function () {
    fetch('/get-applications-count') // Fetch application counts by date
        ...
        .then(data => {
            ...
            ])].sort(); // Get unique dates and sort them

            // Map dates to their respective application counts (default to 0 if no entry)
            const vendorData = dates.map(date => {
                const entry = data.vendors.find(v => v.date === date);
                return entry ? entry.count : 0;
            });

            const supplierData = dates.map(date => {
                const entry = data.suppliers.find(s => s.date === date);
                return entry ? entry.count : 0;
            });

            const subcontractorData = dates.map(date => {
                const entry = data.subcontractors.find(sc => sc.date === date);
                return entry ? entry.count : 0;
            });
            ...
            new Chart(ctx, {
                ...
                data: {
                    labels: dates, // X-axis: Application Dates
                    datasets: [
                        {
                            label: 'Vendors',
                            borderColor: 'blue',
