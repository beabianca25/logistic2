...
@section('content')
                        ...
                        <div class="card-body">
                            <div class="d-flex">
                                <p class="d-flex flex-column">
                                    <span class="text-bold text-lg" id="vendor-count">{{ $vendorCount }}</span>
                                    <span>Vendors Applied</span>
                                </p>
                                <p class="ml-auto d-flex flex-column text-right">
                                    <span class="text-bold text-lg" id="supplier-count">{{ $supplierCount }}</span>
                                    <span>Suppliers Applied</span>
                                </p>
                                <p class="ml-auto d-flex flex-column text-right">
                                    <span class="text-bold text-lg" id="subcontractor-count">{{ $subcontractorCount }}</span>
                                    <span>Subcontractors Applied</span>
                                </p>
                            </div>
                            <!-- /.d-flex -->
    ...
    document.addEventListener("DOMContentLoaded", function () {
        fetch('/get-applications-count') // Fetch data from Laravel route
            .then(response => response.json())
            .then(data => {
                // Update counts
                document.getElementById('vendor-count').innerText = data.vendors;
                document.getElementById('supplier-count').innerText = data.suppliers;
                document.getElementById('subcontractor-count').innerText = data.subcontractors;
    
                var ctx = document.getElementById('applications-chart').getContext('2d');
                new Chart(ctx, {
                    type: 'line',
                    data: {
                        labels: ['18th', '20th', '22nd', '24th', '26th', '28th', '30th'], // Example dates
                        datasets: [
                            {
                                label: 'Vendors',
                                borderColor: 'blue',
                                backgroundColor: 'rgba(0, 123, 255, 0.2)',
                                data: [data.vendors, 90, 110, 120, 140, 130, 150], // Example trend
                                fill: false,
                                tension: 0.4
                            },
                            {
                                ...
                                data: [data.suppliers, 60, 80, 95, 100, 110, 120], // Example trend
                                fill: false,
                                tension: 0.4
                            },
                            {
                                ...
                                data: [data.subcontractors, 40, 50, 70, 80, 90, 100], // Example trend
                                fill: false,
                                tension: 0.4
                            }
                        ]
