                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <form action="{{ route('release.update', $release->id) }}" method="POST">
                            ...
                            <div class="mb-3">
                                <label for="reservation_id_{{ $reservation->id }}" class="form-label">Reservation ID</label>
                                <input type="text" class="form-control" id="reservation_id_{{ $reservation->id }}" 
                                    name="reservation_id[]" value="{{ $reservation->id }}" readonly>
                        ...
                        @endforeach
                            ...
                            <div class="mb-3">
                                <label for="reservation_start_date" class="form-label">Reservation Date</label>
                                <input type="datetime-local" class="form-control" id="reservation_start_date"
                                    name="reservation_start_date"
                                    value="{{ old('reservation_start_date', $release->reservation_start_date ? \Carbon\Carbon::parse($release->reservation_start_date)->format('Y-m-d\TH:i') : '') }}"
                                    required>
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="datetime-local" class="form-control" id="release_date" name="release_date"
                                    value="{{ $release->release_date->format('Y-m-d\TH:i') }}" required>
                            </div>
                            <div class="mb-3">
                                ...
                                <input type="datetime-local" class="form-control" id="drop_off_date" name="drop_off_date"
                                    value="{{ $release->drop_off_date ? $release->drop_off_date->format('Y-m-d\TH:i') : '' }}">
                            </div>
                            <div class="mb-3">
                                <label for="released_by" class="form-label">Released By</label>
                                <input type="text" class="form-control" id="released_by" name="released_by"
                                    value="{{ $release->released_by }}" required>
                            </div>
                            <div class="mb-3">
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-control" id="status" name="status" required>
                                    <option value="Pending" {{ $release->status == 'Pending' ? 'selected' : '' }}>Pending
                                    </option>
                                    <option value="Ongoing" {{ $release->status == 'Ongoing' ? 'selected' : '' }}>Ongoing
                                    </option>
                                    <option value="Completed" {{ $release->status == 'Completed' ? 'selected' : '' }}>
                                        Completed</option>
                                    <option value="Cancelled" {{ $release->status == 'Cancelled' ? 'selected' : '' }}>
                                        Cancelled</option>
                                </select>
                            </div>
                        ...
                        <!-- New Form for Completing Release -->
                        @if ($release->status !== 'Completed')
                            <a href="{{ route('history.index', ['id' => $release->id]) }}">Complete Release</a>

                            <form action="{{ route('release.index', ['id' => $release->id]) }}" method="POST"
                                class="mt-3">
                                @csrf
                                @method('PATCH')
