                                        ...
                                        @foreach ($bids as $bid)
                                            <tr>
                                                ...
                                                <td>
                                                    ...
                                                    @if ($bid->status == 'approved') green
                                                    @elseif($bid->status == 'pending') orange
                                                    @elseif($bid->status == 'winning') red
                                                    @else gray @endif;">
                                                        {{ ucfirst($bid->status) }}
                                                    </span>
                                                </td>
                ...
                <div class="card-body">
                    <div class="d-flex">
                        <p class="d-flex flex-column">
                            <span id="total-bids" class="text-bold text-lg" style="font-size: 0.9rem; font-family: sans-serif;">₱0.00</span>
                            <span style="font-size: 0.9rem; font-family: sans-serif;">Total Bids</span>
                        </p>
                    </div>
        ...
        <script>
            $(document).ready(function() {
                ...
                function fetchBidData() {
                    $.ajax({
                        url: '/get-bid-data', // Laravel route (see below)
                        method: 'GET',
                        dataType: 'json',
                        success: function(response) {
                            ...
                            $('#total-bids').text('₱' + totalBids.toLocaleString());
                        }
                    });
                }
