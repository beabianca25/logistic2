...
@section('content')
    <div class="container" style="font-family: sans-serif; font-size: small;">
                            ...
                            <table class="table m-0">
                                ...
                                <tbody>
                                    @if (isset($bids) && $bids->count() > 0)
                                        @foreach ($bids as $bid)
                                            <tr>
                                                <td><a href="{{ route('bid.show', $bid->id) }}">BID{{ $bid->id }}</a>
                                                </td>
                    ...
                    success: function(response) {
                        ...
                        var barChartData = {
                            ...
                            datasets: [{
                                    ...
                                    data: response.statusCounts['Pending']
                                },
                                {
                                    label: 'Approved',
                                    backgroundColor: 'rgba(75, 192, 192, 0.8)',
                                    data: response.statusCounts['Approved']
                                },
                                {
                                    label: 'Scheduled',
                                    backgroundColor: 'rgba(54, 162, 235, 0.8)',
                                    data: response.statusCounts['Scheduled']
                                },
                                {
                                    label: 'Ongoing',
                                    backgroundColor: 'rgba(153, 102, 255, 0.8)',
                                    data: response.statusCounts['Ongoing']
                                },
                                {
                                    label: 'Completed',
                                    backgroundColor: 'rgba(0, 255, 0, 0.8)',
                                    data: response.statusCounts['Completed']
                                },
                                {
                                    label: 'Cancelled',
                                    backgroundColor: 'rgba(255, 99, 132, 0.8)',
                                    data: response.statusCounts['Cancelled']
                                }
                            ]
                        };
                        new Chart(ctx, {
        ...
        <div class="row">
                                        ...
                                        @foreach ($users as $user)
                                            <li>
                                                <img class="direct-chat-img"
                                                    src="{{ asset('images/JVD-removebg-preview.png') }}"
                                                    alt="User Image">
                                                <a class="users-list-name" href="#">{{ $user->name }}</a>
                                                <span
                                                    class="users-list-date">{{ $user->created_at->format('d M Y') }}</span>
                                            </li>
                                        @endforeach
                                    </ul>
                                </div>
            ...
            <style>
                /* Global card styling for floating effect */
                .card {
