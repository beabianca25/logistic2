...
<head>
    ...
    <title>JVD Travel And Tour</title>
...
<body>
    ...
    <body class="hold-transition sidebar-mini layout-fixed">
            ...
            <nav class="main-header navbar navbar-expand navbar-light navbar-light elevation-1">
                ...
                <ul class="navbar-nav ms-auto d-flex align-items-center">
                    ...
                    <li class="nav-item dropdown me-3">
                        <a class="nav-link d-flex align-items-center" href="#" data-bs-toggle="dropdown">
                            <i class="far fa-bell fw-bold" style="color: #ffcc00; font-size: 1.3rem;"></i>
                            <span id="notifCount" class="badge bg-danger navbar-badge"
                                style="display:none; font-size: 0.8rem;">0</span>
                        </a>
                        <div class="dropdown-menu dropdown-menu-lg dropdown-menu-end">
                            <span class="dropdown-header"><span id="notifHeaderCount">0</span> Notifications</span>
                    ...
                    <script>
                        function loadNotifications() {
                            fetch('/get-notifications')
                                ...
                                .then(data => {
                                    let unreadNotifications = data.filter(notif => !notif.read_at);
                                    let notifCount = unreadNotifications.length;

                                    let notifList = document.getElementById("notifList");
                                    let notifBadge = document.getElementById("notifCount");
                                    let notifHeaderCount = document.getElementById("notifHeaderCount");

                                    notifList.innerHTML = "";
                                    notifHeaderCount.innerText = notifCount;
                                    ...
                                    unreadNotifications.forEach(notif => {
                                        ...
                                        if (notif.data.hasOwnProperty('bid_id')) {
                                            targetUrl = `/bids/${notif.data.bid_id}#notif-${notif.id}`;
                                            notifItem.innerHTML = `
                                                <i class="fas fa-gavel mr-2"></i> ${notif.data.message}
                                                <span class="float-right text-muted text-sm">${new Date(notif.created_at).toLocaleString()}</span>
                                            `;
                                        } else if (notif.data.hasOwnProperty('booking_id')) {
                                            targetUrl = `/bookings/${notif.data.booking_id}#notif-${notif.id}`;
                                            notifItem.innerHTML = `
                                                <i class="fas fa-calendar-check mr-2"></i> ${notif.data.message}
                                                <span class="float-right text-muted text-sm">${new Date(notif.created_at).toLocaleString()}</span>
                                            `;
                                        }

                                        notifItem.href = targetUrl;
                                        notifItem.classList.add("dropdown-item");
                                        notifItem.style.cursor = "pointer";

                                        notifItem.addEventListener("click", function(event) {
                                            event.preventDefault();
                                            markNotificationAsRead(notif.id, targetUrl, notifItem);
                                        });
                        ...
                        function markNotificationAsRead(notificationId, redirectUrl, notifItem) {
                            fetch(`/mark-notification-as-read/${notificationId}`, {
                                ...
                                notifHeaderCount.innerText = newCount;
                                notifBadge.innerText = newCount;
                ...
                <li class="nav-item dropdown pe-3">
                    <a class="nav-link nav-profile d-flex align-items-center pe-0" href="#"
                        ...
                        <span class="d-none d-md-block dropdown-toggle ps-2 fw-bold">
                    ...
                    <ul class="dropdown-menu dropdown-menu-end dropdown-menu-arrow profile">
                        <li class="dropdown-header">
                            <h6 class="fw-bold">{{ Auth::user()->name ?? 'Guest' }}</h6>
                            <span>
                    ...
                    <div class="container" style="font-family: sans-serif; font-size: large;">
                        <nav class="mt-1">
                            <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu"
                                ...
                                <li class="nav-item menu">
                                    @can('view vendor portal')
                                            ...
                                            @can('view vendors')
                                                <li class="nav-item">
                                                    <a href="{{ route('vendor.index') }}" class="nav-link bg-light">
                                                        ...
                                                        <p>Vendor List
                                                            <span class="badge badge-danger" id="vendorCount">
                                                                {{ \App\Models\Vendor::where('status', 'Pending')->count() }}
                                                            </span>
                                                        </p>
                                                    </a>
                                                </li>
                                        ...
                                        <ul class="nav nav-treeview">
                                            @can('view bookings')
                                                <li class="nav-item">
                                                    <a href="{{ route('booking.index') }}" class="nav-link bg-light">
                                                        <i class="far fa-circle text-teal"></i>
                                                        <p>Bookings</p>
                                                    </a>
                                                </li>
                                        ...
                                        <!-- 🔹 Show Auction inside Vendor Portal only for Super Admin -->
                                ...
                                <!-- 🔹 Show Auction outside Vendor Portal for non-Super Admins -->
                                @if (!Auth::user()->hasRole('super admin'))
                                    <li class="nav-item">
                                        @can('view auctions')
                                            <a href="{{ route('auction.index') }}" class="nav-link bg-light">
                                    ...
                                    <li class="nav-item">
                                        ...
                                        <ul class="nav nav-treeview">
                                            ...
                                            @can('view reservation history')
                                                <li class="nav-item">
                                                    <a href="{{ route('history.index') }}" class="nav-link bg-light">
                                                        <i class="far fa-circle text-info"></i>
                                                        <p>History</p>
                                                    </a>
                                                </li>
                                ...
                                @can('view email')
                                    <li class="nav-item">
                                        ...
                                        <ul class="nav nav-treeview">
                                            ...
                                            @can('compose email')
                                                <li class="nav-item">
                                                    <a href="{{ route('email.send') }}" class="nav-link bg-light">
                                                        <i class="far fa-circle text-pink"></i>
                                                        <p>Compose</p>
                                                    </a>
                                                </li>
                                ...
                                @can('view event calendar')
                                    <li class="nav-item">
                                        <a href="{{ route('calendar.index') }}" class="nav-link bg-light">
                                ...
                                <li class="nav-item">
                                    <a href="{{ route('user.booking') }}" class="nav-link bg-light">
                                        <i class="nav-icon fas fa-calendar-check"></i> My Bookings
                                    </a>
                                </li>
                                ...
                                @can('create booking')
                                    <li class="nav-item">
                                        <a href="{{ route('booking.create') }}" class="nav-link bg-light">
                                            <i class="nav-icon fas fa-calendar"></i>
                                            <p>
                                                Book Now!
                                            </p>
                                ...
                                <li class="nav-item">
                                    ...
                                    <ul class="nav nav-treeview">
                                        <li class="nav-item">
                                            <a href="{{ route('vendor.create') }}" class="nav-link bg-light">
                                                <i class="far fa-circle text-teal"></i>
                                                <p>Vendor</p>
                                            </a>
                                        </li>
                                        ...
                                        <li class="nav-item">
                                            <a href="{{ route('subcontractor.create') }}" class="nav-link bg-light">
                                                <i class="far fa-circle text-teal"></i>
                                                <p>Subcontractor</p>
                                            </a>
                                        </li>
                                        ...
                                        <li class="nav-item">
                                            <a href="{{ route('supplier.create') }}" class="nav-link bg-light">
                                                <i class="far fa-circle text-teal"></i>
                                                <p>Supplier</p>
                                            </a>
                                        </li>
        ...
        <script>
            ...
            var pusher = new Pusher("a6578a22d6b06e92beab", {
                cluster: "ap1",
                encrypted: true
            });


            var channel = pusher.subscribe("vendor-channel");

            channel.bind("vendor.application.submitted", function(data) {
                let notifBadge = document.getElementById("notifCount");
                let notifHeaderCount = document.getElementById("notifHeaderCount");
                let notifList = document.getElementById("notifList");
                ...
                notifBadge.style.display = "inline"; // Show the badge

                // Update the dropdown header count
                notifHeaderCount.innerText = currentCount + 1;

                // Add new notification to the dropdown
                let newNotif = `
                    <a href="/vendor" class="dropdown-item">
                        <i class="fas fa-user-plus mr-2 text-success"></i> ${data.message}
                        <span class="float-right text-muted text-sm">Just now</span>
                    </a>
                    <div class="dropdown-divider"></div>
                `;
                notifList.insertAdjacentHTML("afterbegin", newNotif);

                // Optional: Show a toast notification (instead of an alert)
            ...
            function showToastNotification(message) {
                ...
                toast.classList.add("toast", "show", "bg-primary", "text-dark");
                toast.innerHTML = `
