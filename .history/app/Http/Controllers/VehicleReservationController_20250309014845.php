...
class VehicleReservationController extends Controller
{
    public function index()
    {
        ...
        $reservationCount = VehicleReservation::count();
        return view('Reservation.VehicleReservation.index', compact('reservations', 'reservationCount'));
    }
...
{
    $count = VehicleReservation::count();
...
public function publicShow()
{
    ...
    return view('PublicShow.PublicReservation', compact('reservations'));
}

public function getDailyReservationData()
{
    ...
    // Get the date range (e.g., last 30 days)
    ...
    for ($date = strtotime($startDate); $date <= strtotime($endDate); $date += 86400) {
        ...
        $dateRange[$formattedDate] = 0; // Initialize with 0
    }

    // Fill with actual reservation counts
    foreach ($dailyReservations as $date => $count) {
    ...
    return response()->json([
        'dates' => array_keys($dateRange),  // Array of dates
        'counts' => array_values($dateRange) // Corresponding counts
    ]);
}
