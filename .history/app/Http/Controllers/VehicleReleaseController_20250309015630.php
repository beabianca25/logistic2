...
class VehicleReleaseController extends Controller
{
    ...
    public function index()
    {
        ...
        $statusCounts = [
            'Pending' => VehicleRelease::where('status', 'Pending')->count(),
            'Ongoing' => VehicleRelease::where('status', 'Ongoing')->count(),
            'Completed' => VehicleRelease::where('status', 'Completed')->count(),
            'Cancelled' => VehicleRelease::where('status', 'Cancelled')->count(),
        ];

        $releases = VehicleRelease::with('reservation')->get(); // Remove vehicle if not needed
        return view('Reservation.Release.index', compact('releases', 'statusCounts'));
    }
