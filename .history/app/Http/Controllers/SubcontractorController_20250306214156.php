...
class SubcontractorController extends Controller
{
    ...
    public function index()
    {
        if (Auth::user()->role === 'admin') {
            ...
            // Subcontractor can only see their own application
            $subcontractors = Subcontractor::where('user_id', Auth::id())->get();
        }
...
{
    $subcontractor = Subcontractor::with(['requirements', 'attachments'])->findOrFail($id);

    // Debugging: Check if related data exists
    \Log::info('Subcontractor Data:', $subcontractor->toArray());
