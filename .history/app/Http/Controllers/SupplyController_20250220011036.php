    ...
    public function update(Request $request, Supply $supply)
    {
        ...
        if ($quantityChanged !== 0) {
            AuditReport::create([
                ...
                'report_details' => "Stock for '{$supply->supply_name}' was adjusted by {$quantityChanged} units.",
        ...
        if ($supply->remaining_stock <= $supply->reorder_level) {
            AuditReport::create([
                ...
                'report_details' => "Warning: Supply '{$supply->supply_name}' is low with only {$supply->remaining_stock} left.",
                'status' => 'Pending Review',
                'document_status' => 'Pending',
                'submitted_at' => now(),
            ]);
