...
class VendorBookingController extends Controller
{
    ...
    public function userBookings()
    {
        ...
        $bookings = VendorBooking::where('user_id', $userId)
            ->with('vendor')
            ->latest()
            ->get();

        return view('Vendor.Booking.user_index', compact('bookings'));
    }
    ...
    public function vendorBookings()
    {
            ...
            dump(Auth::user()); // Check user details
            dump(Auth::user()->vendor_id); // Check vendor ID
            dd(VendorBooking::where('vendor_id', Auth::user()->vendor_id)->get()); // Check database query results
        ...
        if (!$vendorId) {
            abort(403, 'Unauthorized: You are not a vendor.');
        }
    
        $bookings = VendorBooking::where('vendor_id', $vendorId)
            ->with('user')
            ->latest()
            ->get();
    
        return view('Vendor.Booking.vendor_index', compact('bookings'));
    }
