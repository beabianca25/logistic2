...
class VendorController extends Controller
{
    ...
    public function update(Request $request, $id)
    {
        ...
        // Force status update separately if mass assignment doesn't work
        $vendor->fill($validated);
        if ($vendor->isDirty('status')) { 
            \Log::info('Status has changed from ' . $vendor->getOriginal('status') . ' to ' . $validated['status']);
        } else {
            \Log::warning('Status has NOT changed');
        }
