...
class VendorController extends Controller
{
    ...
    {
        $vendor = auth()->user(); // Get the logged-in vendor
    
        if (!$vendor) {
            return redirect()->route('login')->with('error', 'You must be logged in to view your applications.');
        }
    
        // Retrieve applications where the vendor's email matches the email used in applications
        $applications = Application::where('email', $vendor->email)->get();
    
        return view('Vendor.Vendor.myapp', compact('applications', 'vendor'));
    }
