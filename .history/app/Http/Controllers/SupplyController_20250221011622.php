...
class SupplyController extends Controller
{
    ...
    public function update(Request $request, Supply $supply)
    {
        ...
        Log::info("Supply updated: {$supply->supply_name}, Remaining Stock: {$supply->remaining_stock}, Reorder Level: {$supply->reorder_level}");
    
        if ($supply->remaining_stock <= $supply->reorder_level) {
            Log::info("Stock is below reorder level, checking for existing report...");
            ...
            if ($existingReport) {
                Log::info("Updating existing report for supply ID: {$supply->id}");
                $existingReport->update([
                    'report_details' => "Supply '{$supply->supply_name}' (ID: {$supply->id}) is low with only {$supply->remaining_stock} left.",
                    'submitted_at' => now(),
                ]);
            } else {
                Log::info("Creating new supply report for supply ID: {$supply->id}");
                SupplyReport::create([
                    'supply_id' => $supply->id,
