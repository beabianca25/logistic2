...
class SupplyController extends Controller
{
    ...
    public function update(Request $request, Supply $supply)
    {
        ...
        // Log old and new stock
        Log::info("Updating Supply ID: {$supply->id}, Old Stock: {$supply->remaining_stock}, New Stock: {$request->remaining_stock}");
    
        $supply->update(['remaining_stock' => $request->remaining_stock]);
    
        // Check if stock is below reorder level
        if ($supply->remaining_stock <= $supply->reorder_level) {
            Log::info("Stock below reorder level for Supply ID: {$supply->id}");
            ...
            if ($existingReport) {
                Log::info("Updating existing SupplyReport for Supply ID: {$supply->id}");
                ...
                Log::info("Creating new SupplyReport for Supply ID: {$supply->id}");
            ...
            Log::info("Stock level is sufficient, no report needed for Supply ID: {$supply->id}");
        }
