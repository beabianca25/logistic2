...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('vehicle_releases', function (Blueprint $table) {
            $table->id();
            $table->foreignId('vehicle_id')->constrained()->onDelete('cascade'); // Links to the vehicles table
            $table->string('customer_name');
            $table->string('customer_contact');
            $table->dateTime('reservation_date'); // Date when the booking was made
    ...
    {
        Schema::dropIfExists('vehicle__releases');
    }
};
