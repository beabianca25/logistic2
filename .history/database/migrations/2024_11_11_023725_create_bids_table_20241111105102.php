...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('bids', function (Blueprint $table) {
            $table->id();
            $table->unsignedBigInteger('auction_id'); // References auction item
            $table->unsignedBigInteger('buyer_id');        // References buyer
            $table->decimal('bid_amount', 10, 2);          // Bid amount with 2 decimal precision
            $table->enum('status', ['pending', 'accepted', 'rejected'])->default('pending'); // Bid status// Created at and updated at timestamps
