...
// Vendor Portal
Route::group(['middleware' => ['auth', 'role_or_permission:super-admin|admin|view vendor']], function () {
...
Route::group(['middleware' => ['auth', 'role_or_permission:super-admin|admin|vendor|user']], function () {
...
Route::post('/auction/{auctionId}/bid', [BidController::class, 'store'])
    ->middleware(['auth', 'role_or_permission:super-admin|admin|vendor'])
    ->name('bid.store');

// All authenticated users with the right roles can view the bid index
Route::group(['middleware' => ['auth', 'role_or_permission:super-admin|admin|vendor|user']], function () {
    Route::resource('bid', BidController::class)->only(['index']);
});

// Super Admins, Admins, and Vendors can create and view bids
Route::group(['middleware' => ['auth', 'role_or_permission:super-admin|admin|vendor|user']], function () {
    Route::resource('bid', BidController::class)->only(['create', 'store', 'show']);
});

// Only Super Admins and Admins can edit and delete bids
Route::group(['middleware' => ['auth', 'role_or_permission:super-admin|admin']], function () {
    Route::resource('bid', BidController::class)->only(['edit', 'update', 'destroy']);
});
