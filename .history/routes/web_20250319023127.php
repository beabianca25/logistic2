...
use App\Http\Controllers\VehicleReleaseHistoryController;
...
})->middleware(['auth', 'verified', 'rolemanager:customer'])->name('userdashboard');
...
Route::group(['middleware' => ['auth', 'role_or_permission:super-admin|admin']], function () {
    Route::resource('auction', AuctionController::class)->except(['edit','destroy']);
});
...
Route::get('auction/{auction}/edit', [AuctionController::class, 'edit'])
    ->middleware(['auth', 'role_or_permission:super-admin|admin'])
    ->name('auction.edit');
...
Route::get('/bid/{id}/edit', [BidController::class, 'edit'])->name('bid.edit');
Route::delete('/bid/{id}', [BidController::class, 'destroy'])->name('bid.destroy');
...
Route::put('/bid/{id}', [BidController::class, 'update'])->name('bid.update');

// Only allow authorized users to view the bid list
Route::group(['middleware' => ['auth', 'role_or_permission:super-admin|admin|show bids']], function () {
...
//NOT SURE
Route::get('/bid/count', [BidController::class, 'getBidCount'])->name('bid.count');
...
// Vehicle Management - Only Super Admins and Admins
Route::group(['middleware' => ['auth', 'role_or_permission:super-admin|admin']], function () {
    Route::get('/vehicle', [VehicleController::class, 'index'])->name('vehicle.index'); // List all vehicles
    Route::get('/vehicle/create', [VehicleController::class, 'create'])->name('vehicle.create'); // Create form
    Route::post('/vehicle', [VehicleController::class, 'store'])->name('vehicle.store'); // Store new vehicle
    Route::get('/vehicle/{vehicle}', [VehicleController::class, 'show'])->name('vehicle.show'); // View vehicle
    Route::get('/vehicle/{vehicle}/edit', [VehicleController::class, 'edit'])->name('vehicle.edit'); // Edit form
    Route::put('/vehicle/{vehicle}', [VehicleController::class, 'update'])->name('vehicle.update'); // Update vehicle
    Route::delete('/vehicle/{vehicle}', [VehicleController::class, 'destroy'])->name('vehicle.destroy'); // Delete vehicle
});
...
// Route::group(['middleware' => ['auth', 'role_or_permission:vendor|super-admin|admin|view vendor booking']], function () {
//     Route::get('/vendor/bookings', [VendorBookingController::class, 'vendorBookings'])->name('vendor.bookings');
// });


Route::get('/pending-vendor-count', function () {
    return response()->json(['count' => \App\Models\Vendor::where('status', 'Pending')->count()]);
});



// Display Email Form (Optional)
...
Route::get('/compose-message', function (Illuminate\Http\Request $request) {
...
Route::get('/history', [VehicleReleaseHistoryController::class, 'index'])->name('history.index');

// Route for marking release as completed
Route::patch('/release/{id}/complete', [VehicleReleaseController::class, 'complete'])->name('release.complete');



Route::get('/report/daily-data', [ReportController::class, 'dailyData'])->name('report.dailyData');
...
Route::get('/latest-bids', [BidController::class, 'latestBids'])->name('bid.latest');


Route::resource('booking', BookingController::class);

Route::get('/report/booking-status', [ReportController::class, 'bookingStatusData'])->name('report.bookingStatusData');
Route::get('/get-bid-data', [BidController::class, 'getBidData']);
...
Route::get('/get-notifications', function () {
    ...
    $notifications = $user->notifications; // `notifications` retrieves all, `unreadNotifications` retrieves only unread
...
Route::post('/mark-notification-as-read/{id}', function ($id) {
    ...
    if ($notification) {
        $notification->markAsRead(); // Only mark when the user actually clicks
    }
...
Route::post('/vehicle/{id}/location', [VehicleController::class, 'storeLocation'])->name('vehicle.storeLocation');
Route::get('/vehicle/{id}/locationhistory', [VehicleController::class, 'locationHistory'])->name('vehicle.locationhistory');

Route::get('/api/vehicle-locations', [VehicleController::class, 'getVehicleLocations']);
Route::get('/vehicles/{vehicle}/location', [VehicleController::class, 'locationForm'])->name('vehicle.location');


Route::get('/mybooking', [BookingController::class, 'myBookings'])->name('user.booking');


use App\Http\Controllers\UserDashboardController;

Route::middleware(['auth', 'verified', 'rolemanager:customer,vendor'])->group(function () {
    Route::get('/userdashboard', [UserDashboardController::class, 'index'])->name('userdashboard');
    Route::get('/dashboard/reservation-count', [UserDashboardController::class, 'getReservationCount'])->name('reservation.count');
    Route::get('/dashboard/bid-count', [UserDashboardController::class, 'getBidCount'])->name('bid.count');
    Route::get('/dashboard/vehicle-active-count', [UserDashboardController::class, 'getActiveVehicleCount'])->name('vehicle.active.count');
});


Route::get('/release/ongoing/count', function () {
    return response()->json([
        'ongoingReleaseCount' => \App\Models\VehicleRelease::where('status', 'Ongoing')->count()
    ]);
})->name('release.ongoing.count');
...
Route::post('/mark-notification-as-read/{id}', function ($id, Request $request) {
